...
temp_lp = pl.zeros([enslen,2912])temp_gw.copy()
temp_ox = temp_lp.copy()
...
fig, ax = pl.subplots(2,1,figsize=(16,6))
...
for i in range(len(mds_locs)):
    ...
    axx.plot(Tx_dwr-Tx_mds,marker='o',lw=0.5,label='Tmax',ms=6,color='#ff7f0e')
    axx.plot(Tx_dwr-mx_20cr,lw=0.5,color='k',zorder=5,label='20CRv3 Tmax')
    axx.plot(Tn_dwr[1:]-Tn_mds[:-1],marker='o',lw=0.5,label='Tmin',ms=4,color='#1f77b4')
...
fig.text(0.005,0.93,'(a)',size=12)
fig.text(0.005,0.43,'(b)',size=12)
...
pl.tight_layout()
pl.subplots_adjust(top=0.96,bottom=0.04,wspace=0.16,hspace=0.23,right=0.972)

pl.savefig(jashome+'midas_dwr_20cr_temps_comp_2stations.png',dpi=400)
pl.savefig(jashome+'midas_dwr_20cr_temps_comp_2stations.pdf',dpi=400)