...
fig, ax = pl.subplots(4,3,figsize=(12,10)) # 2x3 = (15,7). 4x3 = (12,10)
...
levs_tct = pl.linspace(-4,4,9)
levs_ns = pl.linspace(-4,4,9)

##############################################################################
ax1 = pl.subplot(431,projection=proj,extent=ext)
...
cs = ax1.contourf(lon2,lat2,signal[0].T,transform=ccrs.PlateCarree(),
                          cmap='BrBG_r',levels=levs_sig,alpha=0.6,extend='both')
...
###############################################################################
ax2 = pl.subplot(432,projection=proj,extent=ext)
ax2.coastlines(linewidth=0.5,resolution='50m')
ax2.add_feature(borders_50m,linewidth=0.5,zorder=5)
cs = ax2.contourf(lon2,lat2,(tc_comp_trnd[0].T)*10,transform=ccrs.PlateCarree(),
                          cmap='BrBG_r',levels=levs_tct,alpha=0.8,extend='both')
ax2.contourf(lon2,lat2,tct_p[0].T,hatches=['...'],colors='none')

GridLines(ax2,False,False,False,False)
#ax2.annotate('(b) JFM NAO component of trend',(-13,68),
#                 bbox={'color':'w'},fontsize=12)
pl.title('(b) '+name+' JFM '+caps[0]+' component of trend',fontsize=12)

###############################################################################
ax3 = pl.subplot(433,projection=proj,extent=ext)
ax3.coastlines(linewidth=0.5,resolution='50m')
ax3.add_feature(borders_50m,linewidth=0.5,zorder=5)
cs = ax3.contourf(lon2,lat2,nosig[0].T,transform=ccrs.PlateCarree(),
                          cmap='BrBG_r',levels=levs_ns,alpha=0.8,extend='both')
ax3.contourf(lon2,lat2,(nosig_p[0].T)*10,hatches=['...'],colors='none')

GridLines(ax3,False,False,False,True)
#ax3.annotate('(c) JFM residual trend',(-13,68),\
#            bbox={'facecolor':'w','alpha':1},fontsize=12)
pl.title('(c) '+name+' JFM residual trend',fontsize=12)

###############################################################################
ax4 = pl.subplot(434,projection=proj,extent=ext)
ax4.coastlines(linewidth=0.5,resolution='50m')
ax4.add_feature(borders_50m,linewidth=0.5,zorder=5)
cs = ax4.contourf(lon2,lat2,signal[1].T,transform=ccrs.PlateCarree(),
                          cmap='BrBG_r',levels=levs_sig,alpha=0.8,extend='both')
ax4.contourf(lon2,lat2,sig_p[1].T,hatches=['...'],colors='none')
#ax4.annotate('(d) JFM EA signal',(-13,68),bbox={'facecolor':'w'},fontsize=12)
pl.title('(d) '+name+' JFM '+caps[1]+' signal',fontsize=12)

#f = pl.gcf()
#colax = f.add_axes([0.063,0.075,0.275,0.03])
...
GridLines(ax4,False,False,True,False)

###############################################################################
ax5 = pl.subplot(435,projection=proj,extent=ext)
ax5.coastlines(linewidth=0.5,resolution='50m')
ax5.add_feature(borders_50m,linewidth=0.5,zorder=5)
cs = ax5.contourf(lon2,lat2,(tc_comp_trnd[1].T)*10,transform=ccrs.PlateCarree(),
                          cmap='BrBG_r',levels=levs_tct,alpha=0.8,extend='both')
ax5.contourf(lon2,lat2,tct_p[1].T,hatches=['...'],colors='none')
pl.title('(e) '+name+' JFM '+caps[1]+' component of trend',fontsize=12)

#f = pl.gcf()
#colax = f.add_axes([0.375,0.075,0.25,0.03])
#cb = pl.colorbar(cs,orientation='horizontal',cax=colax)
#cb.set_label('mm '+'year $^{-1}$',fontsize=12,labelpad=-1)
#cb.set_ticks(levs_tct)
#cb.set_ticklabels(levs_tct.astype(int))
##cb.ax.tick_params(labelsize=9) 
GridLines(ax5,False,False,False,False)
#ax5.annotate('(e) JFM EA component of trend',(-13,68),bbox={'facecolor':'w'},fontsize=12)

###############################################################################
ax6 = pl.subplot(436,projection=proj,extent=ext)
ax6.coastlines(linewidth=0.5,resolution='50m')
ax6.add_feature(borders_50m,linewidth=0.5,zorder=5)
cs = ax6.contourf(lon2,lat2,(nosig[1].T)*10,transform=ccrs.PlateCarree(),
                          cmap='BrBG_r',levels=levs_ns,alpha=0.8,extend='both')
ax6.contourf(lon2,lat2,nosig_p[1].T,hatches=['...'],colors='none')
pl.title('(f) '+name+' JFM residual trend',fontsize=12)


#f = pl.gcf()
#colax = f.add_axes([0.675,0.075,0.25,0.03])
#cb = pl.colorbar(cs,orientation='horizontal',cax=colax)
#cb.set_label('mm '+'year $^{-1}$',fontsize=12,labelpad=-1)
...
###############################################################################
ax7 = pl.subplot(437,projection=proj,extent=ext)
ax7.coastlines(linewidth=0.5,resolution='50m')
ax7.add_feature(borders_50m,linewidth=0.5,zorder=5)
cs = ax7.contourf(lon2,lat2,signal[2].T,transform=ccrs.PlateCarree(),
                          cmap='BrBG_r',levels=levs_sig,alpha=0.8,extend='both')
ax7.contourf(lon2,lat2,sig_p[2].T,hatches=['...'],colors='none')
#ax4.annotate('(d) JFM EA signal',(-13,68),bbox={'facecolor':'w'},fontsize=12)
pl.title('(g) '+name+' JFM '+caps[2][:3]+' signal',fontsize=12)

#f = pl.gcf()
#colax = f.add_axes([0.117,0.055,0.23,0.03])
#cb = pl.colorbar(cs,orientation='horizontal',cax=colax)
#cb.set_label('$^\circ$C '+'index $^{-1}$',fontsize=12,labelpad=-1)
#cb.set_ticks(levs_sig[::2])
#cb.set_ticklabels(levs_sig[::2].astype(int))
#cb.ax.tick_params(labelsize=10)
GridLines(ax7,False,False,True,False)

##############################################################################
ax8 = pl.subplot(438,projection=proj,extent=ext)
ax8.coastlines(linewidth=0.5,resolution='50m')
ax8.add_feature(borders_50m,linewidth=0.5,zorder=5)
cs = ax8.contourf(lon2,lat2,(tc_comp_trnd[2].T)*10,transform=ccrs.PlateCarree(),
                          cmap='BrBG_r',levels=levs_tct,alpha=0.8,extend='both')
ax8.contourf(lon2,lat2,tct_p[2].T,hatches=['...'],colors='none')
pl.title('(h) '+name+' JFM '+caps[2][:3]+' component of trend',fontsize=12)

#f = pl.gcf()
#colax = f.add_axes([0.382,0.055,0.23,0.03])
#cb = pl.colorbar(cs,orientation='horizontal',cax=colax)
#cb.set_label('$^\circ$C '+'year $^{-1}$',fontsize=12,labelpad=-1)
#cb.set_ticks(levs_tct)
#cb.set_ticklabels(levs_tct.astype(int))
#cb.ax.tick_params(labelsize=9) 
GridLines(ax8,False,False,False,False)
...
pl.title('(i) '+name+' JFM residual trend',fontsize=12)
...
#ax4.annotate('(d) JFM EA signal',(-13,68),bbox={'facecolor':'w'},fontsize=12)
pl.title('(j) '+name+' JFM '+caps[3]+' signal',fontsize=12)

f = pl.gcf()
colax = f.add_axes([0.10,0.055,0.23,0.025])
cb = pl.colorbar(cs,orientation='horizontal',cax=colax)
cb.set_label('days '+'index $^{-1}$',fontsize=12,labelpad=-1)
cb.set_ticks(levs_sig[::2])
...
###############################################################################
ax11 = pl.subplot(4,3,11,projection=proj,extent=ext)
ax1.coastlines(linewidth=0.5,resolution='50m')
ax11.add_feature(borders_50m,linewidth=0.5,zorder=5)
cs = ax11.contourf(lon2,lat2,(tc_comp_trnd[3].T)*10,transform=ccrs.PlateCarree(),
                          cmap='BrBG_r',levels=levs_tct,alpha=0.8,extend='both')
ax11.contourf(lon2,lat2,tct_p[3].T,hatches=['...'],colors='none')
pl.title('(k) '+name+' JFM '+caps[3]+' component of trend',fontsize=12)

f = pl.gcf()
colax = f.add_axes([0.395,0.055,0.23,0.025])
cb = pl.colorbar(cs,orientation='horizontal',cax=colax)
cb.set_label('days '+'decade $^{-1}$',fontsize=12,labelpad=-1)
cb.set_ticks(levs_tct)
cb.set_ticklabels(levs_tct)
#cb.ax.tick_params(labelsize=9) 
GridLines(ax11,False,True,False,False)
...
pl.title('(l) '+name+' JFM residual trend',fontsize=12)
...
###############################################################################
#pl.tight_layout()
pl.subplots_adjust(top=0.96,wspace=-0.23,left=0.020,right=1.0)

#pl.savefig(indecis+'figures/'+name+'_'+teleindex[0]+'_'+teleindex[1]+'_'+teleindex[2]+'_'+teleindex[3]+'_ao_panels.png',dpi=400)

"""fig, ax  = pl.subplots(2,1,figsize=(6,6))

ax1 = pl.subplot(211,projection=proj,extent=ext)
ax1.coastlines(linewidth=0.5,resolution='50m')
ax1.add_feature(borders_50m,linewidth=0.5,zorder=5)
cs = ax1.contourf(lon2,lat2,signal[0].T,transform=ccrs.PlateCarree(),
                  cmap='BrBG_r',levels=levs_sig,alpha=0.8,extend='both')
...
ax1.annotate('ogs10 JFM NAO signal',(-13,68),bbox={'fc':'w'},fontsize=12)

ax2 = pl.subplot(212,projection=proj,extent=ext)
ax2.coastlines(linewidth=0.5,resolution='50m')
ax2.add_feature(borders_50m,linewidth=0.5,zorder=5)
...
GridLines(ax2,False,True,True,False)
ax2.annotate('ogs10 JFM EA signal',(-13,68),bbox={'fc':'w'},fontsize=12)
#pl.tight_layout()

f = pl.gcf()
colax = f.add_axes([0.85,0.2,0.05,0.6])
cb = pl.colorbar(cs,orientation='vertical',cax=colax)
cb.set_label('days index$^{-1}$',fontsize=12,labelpad=-1)
cb.set_ticks(levs_sig)
cb.set_ticklabels(levs_sig.astype(int))

pl.subplots_adjust(top=0.95,bottom=0.05,left=0.00,right=0.95,hspace=0.1)

#pl.savefig(indecis+'figures/graphical_abstract.tif',dpi=351)
