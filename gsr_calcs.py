...
data_gsr = ncfile.variables[name1][:]
data_gsr = xr.DataArray(data_gsr)
...
data_rti = ncfile.variables[name2][:]
data_rti = xr.DataArray(data_rti)
...
data_gsr = data_gsr[:,70:326,34:200]
data_rti = data_rti[:,70:326,34:200]
data_rti = pl.reshape(data_rti.data,
                      newshape=(data_gsr.shape[0],
                                data_rti.shape[0]/data_gsr.shape[0],
                                                        lon2.size,lat2.size))

rti_std = pl.nanstd(data_rti[:,3:10,:,:],axis=0)
...
for m in range(3,10):
    for i in range(lon2.size):
        for j in range(lat2.size):
            r = stats.pearsonr(data_gsr[:,i,j],data_rti[:,m,i,j])
            cors[m-3,i,j] = r[0]
            #out = stats.linregress(x,data_rti[:,m,i,j])
...
for m in range(len(ig)):
    ...
    cs = axx.contourf(lon2,lat2,cors[m].T,transform=ccrs.PlateCarree(),levels=clevs,
                  cmap=cmap,extend='max',alpha=0.6)
                  #norm=MidpointNormalize(midpoint=0))
    axx.contourf(lon2,lat2,pval[m].T,hatches=['..'],colors='none',ec='k')
...
p#l.savefig(indecis+'figures/'+name1+'_'+name2+'_monthly_cors.png',dpi=407)