...
nao = pl.reshape(nao[:,-1],newshape=(nao.shape[0]/12,12))
nao_sns = pl.zeros([nao.shape[0],4])
nao_sns[:,1] = pl.mean(nao[:,2:5],axis=1) # MAM
nao_sns[:,2] = pl.mean(nao[:,5:8],axis=1) # JJA
nao_sns[:,3] = pl.mean(nao[:,8:11],axis=1) # SON
nao_sns[1:,0] = (nao[:-1,-1]+nao[1:,0]+nao[1:,1])/3 # DJF
nao_sns[0,0] = pl.float32('nan')

nao_sns = pl.reshape(nao_sns,newshape=(nao_sns.shape[0]*nao_sns.shape[1]))
...
for i in range(lon.shape[0]):
    for j in range(lat.shape[0]):
        C[i,j] = stats.pearsonr(nao_sns[ind:-8:4],data.values[ind::4,i,j])[0]
        trend[i,j] = stats.linregress(pl.linspace(1,nao_sns[ind:-8:4].shape[0],
                                                nao_sns[ind:-8:4].shape[0]),
                                                data.values[ind::4,i,j])[0]
...
pl.title('$ET_{o}$ MAM NAO correlation',y=1.01)
ax.annotate(' max = '+str(round(Cmax,2))+'\n min = '+str(round(Cmin,2)),
                        (-34,56))

pl.savefig(indecis+'corrmaps/'+name+'_mam_corr.png')
...
GridLines(ax,False,True,True,True)
pl.subplots_adjust(top=0.99,bottom=0.0,left=0.08,right=0.92)
pl.title('$ET_{o}$ MAM trend',y=1.01)

pl.savefig(indecis+'trndmaps/'+name+'_mam_trnd.png')