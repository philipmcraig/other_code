...
lon2 = lon.values[61:326]
lat2 = lat.values[34:187]
#data_mn = pl.nanmean(data.values[:,61:326,34:187],axis=0)
...
areas_clip = areas[61:326,34:187]
data_clip = data.values[ind:,61:326,34:187]

rdata = data_clip[:,:,:]*rmask[None,:,:]
rareas = areas_clip*rmask

Q = pl.ones_like(data_clip)
f = pl.isnan(data_clip)
d = pl.where(f==True)
Q[d[0],d[1],d[2]] = pl.float32('nan')

#P = pl.average(rdata[0],weights=pl.nan_to_num(rareas))
W = pl.zeros([data_clip.shape[0]+1])
W[0] = pl.float32('nan')
 
for i in range(1,data_clip.shape[0]+1): # loop over seasons
    W[i] = pl.nansum(rdata[i-1]*rareas)/(pl.nansum(rareas*Q[i-1]))
...
#ax1 = pl.subplot(111)
for i in range(4):
    pl.plot(pl.linspace(1950,2017,68),W[:,i],label=sns_labs[i],
            color=c[i],ls=ls[i],zorder=zo[i],lw=lw[i])
...
inx = tele_sns.shape[0]-(ind*4)/12
print stats.pearsonr(W[1:,0],tele_sns[1:,0])
for s in range(1,4):
    cor = stats.pearsonr(W[:,s],tele_sns[:,s])
    print cor